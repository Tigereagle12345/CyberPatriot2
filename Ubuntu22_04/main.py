...
class Log():
    ...
    def text(self, text):
        print(f"{self.BOLD}{logging.debug(text)}")
    
    def warn(self, text):
        print(f"{self.WARNING}{logging.warning(text)}")
        
    def error(self, text):
        print(f"{self.FAIL}{logging.error(text)}")
    
    def done(self, text):
        print(f"{self.OKGREEN}{logging.info(text)}")

    def updateLevel(level=logging.DEBUG):
...
def mainScript(log, CURR_DIR, SYSTEM, USERS, USERNAMES, OSTYPE, USERFILE, ADMINFILE, MASTER_PASSWORD, CURR_USER):
    ...
    if not WINDOWS or LINUX:
        ...
        log.text("Starting Ubuntu Script...")
        ubuntu2104(log, CURR_DIR, USERS, USERNAMES, USERFILE, ADMINFILE, OSTYPE, MASTER_PASSWORD, CURR_USER)
