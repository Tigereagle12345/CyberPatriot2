...
def ubuntu2204(log, CURR_DIR, USERS, USERNAMES, USERFILE, ADMINFILE, OSTYPE, MASTER_PASSWORD):
    ...
    log.normal("Refreshing/Reinstalling default apt repositories...")

    log.normal("Installing dependancies for add-apt-repository...")
    os.system("apt install software-properties-common -y")

    log.normal("Installing 'main' repository...")
    os.system("add-apt-repository main -y")
    log.done("Done!")

    log.normal("Installing 'universe' repository...")
    os.system("add-apt-repository universe -y")
    log.done("Done!")

    log.normal("Installing 'restricted' repository...")
    os.system("add-apt-repository restricted -y")
    log.done("Done!")

    log.normal("Installing 'multiverse' repository...")
    ...
    log.normal("Ensuring /tmp is a seperate partition...")
    ...
    log.normal("Disabling automounting...")
    ...
    log.normal("Disabling root account...")
    os.system("passwd root -l")
    log.done("Root account disabled!")

    # Ensure address space layout randomization (ASLR) is enabled
    log.normal("Ensuring ASLR is enabled")
    ...
    log.normal("Ensuring XDCMP is not enabled...")
    ...
    # Remove rsync
    log.normal("Uninstalling rsync...")
    os.system("apt purge rsync -y")
    log.done("Rsync uninstalled!")

    # Remove rsh
    log.normal("Uninstalling rsh...")
    ...
    log.normal("Disabling the su command...")
    ...
    # Find unowned files
    log.normal("Finding unowned files...")
    ...
    for user in USERS:
        if user.pid > 999:
            if not os.path.isdir(f"/home/{user.name}"):
                log.normal(f"Creating a home directory for {user.name}...")
    ...
    log.normal("Removing unauthorized .netrc, .forward and .rhost files...")
...
def firefox(log, USERS, CURR_DIR):
    ...
    if answer("Should firefox be installed as a snap package?", log):
        ...
        log.normal("Removing the firefox snap package...")
        os.system("snap remove firefox")
        log.done("Firefox snap package removed!")
        
        log.normal("Installing firefox as a .deb package...")
        os.system("apt install firefox -y")
...
def permissions(log):
    ...
    for file in files1:
        log.normal(f"Setting permissions on {file}...")
...
def passwd(log, CURR_DIR, USERS, USERNAMES, MASTER_PASSWORD):
    ...
    log.normal("Setting password complexity requirements...")
    ...
    log.normal("Enabling lockout for failed password attempts...")
    ...
    log.normal("Setting hashing algorithm to yescrypt...")
    ...
    for user in USERNAMES:
        if answer(f"Change password for {user} to the master password?", log):
            log.normal(f"Changing {user}'s password...")
            ...
            while run:
                ...
                if answer(f"Should {user}'s password be changed to {password}?", log):
                    ...
                    log.normal("Ok, trying again...")

    # Configure /etc/login.defs
    log.normal("Configuring /etc/login.defs...")
    ...
    log.normal("Removing users from the shadow group...")
...
def cron(log):
    ...
    log.normal("Setting permissions on /etc/cron.hourly...")
    ...
    log.normal("Setting permissions on /etc/cron.daily...")
    ...
    log.normal("Setting permissions on /etc/cron.weekly...")
    ...
    log.normal("Setting permissions on /etc/cron.monthly...")
    ...
    log.normal("Setting permissions on /etc/cron.d...")
...
def audit(log, CURR_DIR):
    ...
    with open("/etc/audit/auditd.conf", "w") as file:
        ...
        log.normal("Ensuring logs are not automatically deleted...")
        fileText = fileText + "\nmax_log_file_action = keep_logs"
        log.done("Logs will not be automatically deleted!")

        # Ensure auditd stops the system when log files are full
        log.normal("Ensuring auditd stops the system when log files are full...")
    ...
    # Set auditd rules
    log.normal("Setting rules for auditd...")
    ...
    log.normal("Setting permissions on log files to 640...")
...
def ipRules(log):
    with open("/etc/ufw/sysctl.conf", "w") as file:
        ...
        log.normal("Disabling packet redirect sending...")
        ...
        log.normal("Disabling IP forwarding...")
        ...
        log.normal("Ensuring source routed packets are not accepted...")
        if "net.ipv4.conf.all.accept_source_route=" in fileText:
            re.sub(r"net.ipv4.conf.all.accept_source_route=\d", "net.ipv4.conf.all.accept_source_route=0", fileText)
        else:
            fileText = fileText + "\nnet.ipv4.conf.all.accept_source_route=0"
        ...
        # Deny ICMP redirects
        log.normal("Denying ICMP redirects...")
        ...
        log.normal("Logging suspicious packets...")
        ...
        log.normal("Ignoring broadcast ICMP requests...")
        ...
        log.normal("Ignoring bogus ICMP responses...")
        ...
        log.normal("Enabling Reverse Path Filtering...")
        ...
        log.normal("Enabling TCP SYN Cookies...")
        ...
        log.normal("Denying IPv6 router advertisements...")
...
# Delete services
def delServices(log):
    log.normal("Deleting possibly dangerous services...")
    
    # Uninstall XServer
    if answer("Should XServer be installed?", log):
        log.normal("Uninstalling XServer...")
        os.system("apt purge xserver-xorg* -y")
        log.normal("XServer uninstalled!")

    # Uninstall Avahi Server
    if answer("Should Avahi Server be installed?", log):
        log.normal("Uninstalling Avahi Server...")
...
def gdm(log):
    ...
    log.normal("Configuring GDM login profile settings...")
    ...
    log.normal("Configuring GDM login banner message...")
    ...
    log.normal("Disabling user list on the login screen...")
    ...
    log.normal("Forcing the screen to lock when the user is idle..")
    ...
    log.normal("Disabling USB automounting...")
    ...
    log.normal("Ensuring GDM autorun-never is enabled...")
    ...
    # Update GDM settings
    log.normal("Updating GDM settings...")
    os.system("dconf update")
    log.done("Updated GDM settings!")

# Configure warning banners
def warningBanner(log):
    log.normal("Configuring warning banner...")
    # Configure local warning banner
    log.normal("Configuring local warning banner...")
    ...
    log.normal("Configuring remove warning banner...")
    ...
    log.normal("Configuring permissions for local warning banners...")
    ...
    log.normal("Configuring permissions for remote warning banners...")
...
# AppArmour
def appArmour(log):
    log.normal("Installing and configuring AppArmour...")
    # Install AppArmour
    log.normal("Installing AppArmour...")
    os.system("apt install apparmor -y")
    log.done("Done!")

    # Configure AppArmour
    log.normal("Configuring AppArmour...")
    ...
    log.normal("Setting AppArmour profiles to enforce...")
...
def restrictCoredumps(log):
    log.normal("Ensuring coredumps are restricted...")
...
def bootloaderPass(log, MASTER_PASSWORD, CURR_DIR):
    log.normal("Setting bootloader password...")
    ...
    log.normal("Writing config to custom grub file...")
    ...
    # Restarting grub
    log.normal("Restarting grub...")
    os.system("update-grub")
    log.done("Grub has been restarted!")

    log.normal("Setting permissions on /etc/grub/grub.cfg...")
...
def aide(log, CURR_DIR):
    # Install AIDE
    log.normal("Installing AIDE...")
    os.system("apt install aide aide-common -y")
    log.done("Done!")

    # Copy basic configuration to /etc/aide/aide.conf
    log.normal("Copying basic configuration to /etc/aide/aide.conf...")
...
def ufw(log):
    ...
    # Disable UFW Looping
    log.normal("Disabling UFW looping...")
    ...
    for user in goodUsers:
        if user not in USERNAMES:
            if answer(f"Unauthorized user {user} detected: Remove?", log):
                ...
                log.normal(f"Proceeding without deleting user {user}.")
        ...
        for admin in GROUPS["adm"]["Users"]:
            if admin not in goodAdmins:
                if answer(f"Unauthorized user {admin} in the adm group: Remove?", log):
                    ...
                    log.normal(f"Proceeding without removing {admin} from the adm group.")
        ...
        for admin in GROUPS["sudo"]["Users"]:
            if admin not in goodAdmins:
                if answer(f"Unauthorized user {admin} in the sudo group: Remove?", log):
                    ...
                    log.normal(f"Proceeding without removing {admin} from the sudo group.")
